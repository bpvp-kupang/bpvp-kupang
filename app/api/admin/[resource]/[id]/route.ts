...
const { resource: res, id } = await params;
...
if (!sameOrigin(req)) {
return jerr("Origin tidak diizinkan (proteksi CSRF).", 403);
}
...
if (!Object.keys(picked).length) {
  return jerr("Tidak ada perubahan.", 400);
}

const data = (await cfg.before?.(picked, false)) ?? picked;

const row = await (prisma as any)[cfg.model].update({
  where: { id },
  data,
});
...
return jerr("Data tidak ditemukan.", 404);
}

if (String(e?.code) === "P2002") {
  return jerr("Nama/email tersebut sudah digunakan.", 400);
}
...
if (!can(s.role, res)) {
return jerr(
"Peran Anda tidak berhak menghapus data ini.",
403
);
}

if (res === "users" && id === s.id) {
return jerr("Tidak dapat menghapus akun sendiri.", 400);
}

try {
const row = await (prisma as any)[cfg.model].delete({
where: { id },
});
...
return jerr(
"Data ini masih terhubung dengan data lain (mis. batch/pendaftar). Nonaktifkan atau hapus data terkait lebih dulu.",
400
);
}

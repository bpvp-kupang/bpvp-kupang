...
export async function GET() {
  ...
  return new NextResponse(JSON.stringify(dump, null, 2), {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="bpvpkupang-backup-${new Date()
        .toISOString()
        .slice(0, 10)}.json"`,
    },
  });
...
export async function POST(req: NextRequest) {
  ...
  if (!sameOrigin(req)) {
    return NextResponse.json(
      { error: "Origin tidak diizinkan (proteksi CSRF)." },
      { status: 403 }
    );
  }
  ...
  if (!dump?.tables?.user || !dump?.tables?.trainingProgram) {
    return NextResponse.json(
      { error: "Struktur file backup tidak dikenal." },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    await prisma.auditLog.create({
      data: {
        actor: s.name,
        action: "Restore",
        detail: "Memuat data dari file backup",
      },
    });

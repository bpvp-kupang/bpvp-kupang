  ...
  return (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-line">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center gap-7 h-[74px]">
        ...
        >
          <div className="w-11 h-11 rounded-xl bg-navy grid place-items-center text-orange font-display font-extrabold text-xl">
            B
          </div>
        ...
        <div className="flex items-center gap-2.5 ml-auto lg:ml-0">
          ...
          <button
            className="lg:hidden w-[42px] h-[42px] grid place-items-center rounded-lg border border-line"
      ...
      {open && (
        <div className="fixed inset-0 z-[60] lg:hidden">
          ...
          <nav
            className="absolute right-0 top-0 bottom-0 w-[min(330px,86vw)] bg-white p-5 overflow-auto"
            aria-label="Menu lengkap"
          >

            <div className="flex items-center justify-between mb-3">
              <b className="font-display text-[16px] text-navy">
                Menu
              </b>

              <button
                className="w-10 h-10 grid place-items-center rounded-lg border border-line"
                onClick={() => setOpen(false)}
            ...
            {ALL.map(([href, label]) => (
              <Link
                ...
                href={href}
                onClick={() => setOpen(false)}
                className="block font-semibold px-3 py-3.5 rounded-lg hover:bg-sky"
              >
                {label}
              </Link>
            ))}

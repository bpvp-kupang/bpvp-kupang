...
export const RES: Record<string, ResCfg> = {
  programs:     { model: "trainingProgram", where: { status: "aktif" }, order: { name: "asc" }, allow: keys("program"), label: (r) => r.name },
  batches:      { model: "trainingBatch", where: { year: 2026, program: { status: "aktif" } }, include: { program: true }, order: { regStart: "asc" }, allow: keys("jadwal"), label: (r) => `${r.program?.name ?? ""} — ${r.name}` },
